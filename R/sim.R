...
# Functions to support replicated simulations of VS and VE data.


rep_VSE <- function(n, vs_model, pred_data, map = NULL, sd_plot = TRUE, sd_obs = TRUE, cl = NULL, type = "txt")
{
  # Simulates virtual species (VS) and virtual ecologist data (VE) from the fitted GAMM. 
  ...
  # 
  # The randomness can be introduced at three levels: 
  #   1) while generating random intercepts for sampling plot identifiers (can be disabled 
  #      by setting the 'sd_plot' argument to zero), 
  #   2) when drawing a random sample from the Tweedie distribution, 
  #   3) when allocating random sampling error to individual observers (can be disabled 
  #      by setting the 'sd_obs' argument to zero). 
  ...
  #   sd_plot: standard deviation of the random intercepts representing the sampling plot
  #            effect (if not TRUE, it is set to zero)
  #    sd_obs: standard deviation of the random intercepts representing the observer effect
  #            (if not TRUE, it is set to zero)
  ...
  sd_plot <- if(sd_plot) gv["s(plot_id)", "std.dev"] else 0 # sd for plot identifiers
  sd_obs <- if(sd_obs) gv["s(observer_id)", "std.dev"] else 0 # sd for observer identifiers
  ...
  w <- pbreplicate(n, {
    ...
    dta[, ve := vs * obs_r]
    idx <- match(pred_data[["id_year"]], dta[["id_year"]])
    dta <- dta[idx, ]
    cbind(dta[["vs"]], dta[["ve"]])
  }, 
  cl = cl)
